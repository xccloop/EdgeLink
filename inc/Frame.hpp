...
constexpr uint8_t FRAME_VERSION = 0x03;
...
constexpr unsigned int FRAME_PRESSURE_LENGTH = 4;
constexpr unsigned int FRAME_PRESSURE_SCALE_LENGTH = 1;
constexpr unsigned int FRAME_TELEMETRY_LENGTH = FRAME_TEMPERATURE_LENGTH
    + FRAME_TEMPERATURE_SCALE_LENGTH + FRAME_PRESSURE_LENGTH + FRAME_PRESSURE_SCALE_LENGTH;
...
/*
    ...
    Version    V3 固定 0x03；V1、V2都是旧布局，不能混用
    ...
    Pressure         int32，大端
    PressureScale    int8，实际气压 = Pressure * 10^PressureScale
...
static_assert(sizeof(FrameHeader) == FRAME_HEADER_LENGTH, "FrameHeader must be 7 bytes");
