...
bool Storage::insertMessage(const Message& msg)
{
    const char* sql = "INSERT INTO messages (nodeId, sequence, temperature, temperatureScale, receivedAtUs) VALUES (?, ?, ?, ?, ?);";
    sqlite3_stmt* stmt = nullptr;
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        lastError_ = sqlite3_errmsg(db_);
    ...
    sqlite3_bind_int(stmt, 2, msg.sequence);
