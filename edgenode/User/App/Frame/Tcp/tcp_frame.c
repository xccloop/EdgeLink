...
uint8_t tcp_frame_encode(uint8_t frame[16],uint16_t sequence,int32_t temperature,int8_t temperature_scale)
{
    ...
    frame[7]  = (uint8_t)((uint32_t)temperature >> 24);
    frame[8]  = (uint8_t)((uint32_t)temperature >> 16);
    frame[9]  = (uint8_t)((uint32_t)temperature >> 8);
    frame[10] = (uint8_t)temperature;
    frame[11] = (uint8_t)temperature_scale;
